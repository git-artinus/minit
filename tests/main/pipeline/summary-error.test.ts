...
import { classifyClaudeFailure } from '../../../src/main/pipeline/summary-error'
import { InvalidOutputError } from '../../../src/main/pipeline/summarizer'
import type { SummaryFailure } from '../../../src/shared/types'

// context는 로그 문구에만 쓰이고 판정에는 관여하지 않는다 — 케이스마다 반복하지 않도록 고정한다.
const classify = (e: unknown): SummaryFailure => classifyClaudeFailure(e, '요약')

// classifyClaudeFailure는 진단 전문을 console.error로 남긴다(원인 영구 소실 방지). 출력만 죽인다.
beforeEach(() => {
...
describe('classifyClaudeFailure — 사유 판정', () => {
  test('spawn ENOENT는 미설치', () => {
    expect(classify(runError({ errorCode: 'ENOENT', exitCode: null })).reason).toBe(
      'not_installed'
    )
  })
  ...
  test('로그인 안 됨(stdout)은 미인증', () => {
    expect(
      classify(runError({ stdout: 'Not logged in · Please run /login' })).reason
    ).toBe('not_authenticated')
  })

  test('API 키 무효도 미인증', () => {
    expect(
      classify(runError({ stdout: 'Invalid API key · Fix external API key' })).reason
    ).toBe('not_authenticated')
  })

  test('대소문자를 가리지 않는다', () => {
    expect(classify(runError({ stdout: 'NOT LOGGED IN' })).reason).toBe(
      'not_authenticated'
    )
  })

  test('사용량 문구는 usage_limit', () => {
    expect(classify(runError({ stdout: 'Usage limit reached' })).reason).toBe(
      'usage_limit'
    )
    expect(classify(runError({ stderr: 'Credit balance too low' })).reason).toBe(
      'usage_limit'
    )
  })

  test('타임아웃은 출력이 비어 있어도 timeout으로 가른다', () => {
    const f = classify(timedOut())
    expect(f.reason).toBe('timeout')
    expect(f.detail).toContain('제한 시간')
  })

  test('JSON 파싱 실패는 invalid_output이고 응답 원문을 남긴다', () => {
    const f = classify(new InvalidOutputError('죄송합니다. 요약을 만들 수 없었습니다.'))
    expect(f.reason).toBe('invalid_output')
    expect(f.detail).toContain('요약을 만들 수 없었습니다')
  })

  test('매칭되지 않으면 unknown이고 원문을 그대로 보존한다', () => {
    const f = classify(runError({ stderr: 'error: unknown option' }))
    expect(f.reason).toBe('unknown')
    expect(f.detail).toContain('error: unknown option')
  })
})

describe('classifyClaudeFailure — 오분류 방지', () => {
  ...
  test('긴 모델 출력에 authentication이 섞여도 미인증으로 오분류하지 않는다', () => {
    ...
    expect(classify(runError({ stdout: modelText })).reason).toBe('unknown')
  })
  ...
  test('JSON 출력은 키워드 검사에서 제외한다', () => {
    ...
    expect(classify(runError({ stdout: json })).reason).toBe('unknown')
  })
  ...
  test('펜스로 감싼 JSON도 키워드 검사에서 제외한다', () => {
    ...
    expect(classify(runError({ stdout: fenced })).reason).toBe('unknown')
  })
  ...
  test('프록시 인증 요구는 미인증으로 오분류하지 않는다', () => {
    expect(classify(runError({ stderr: 'proxy authentication required' })).reason).toBe(
      'unknown'
    )
    expect(classify(runError({ stderr: '401 Unauthorized' })).reason).toBe('unknown')
  })
  ...
  test('진단 길이 경계(400자) 전후로 판정이 갈린다', () => {
    ...
    expect(classify(runError({ stdout: under })).reason).toBe('not_authenticated')
    expect(classify(runError({ stdout: over })).reason).toBe('unknown')
  })
})

describe('classifyClaudeFailure — detail 생성', () => {
  test('프롬프트 전문을 담지 않는다', () => {
    const f = classify(runError({ stdout: 'Not logged in' }))
    expect(f.detail).not.toContain('Command failed')
    expect(f.detail).not.toContain('-p')
  })

  test('stdout·stderr에 서로 다른 정보가 있으면 둘 다 담는다', () => {
    const f = classify(runError({ stdout: '출력쪽 내용', stderr: '오류쪽 내용' }))
    expect(f.detail).toContain('출력쪽 내용')
    expect(f.detail).toContain('오류쪽 내용')
  })

  test('출력이 전혀 없으면 exit code를 알려준다', () => {
    expect(classify(runError({ exitCode: 7 })).detail).toContain('exit code 7')
  })

  test('길면 앞뒤를 남기고 잘렸음을 알린다', () => {
    const long = `머리말${'가'.repeat(2000)}진짜원인은맨뒤에있다`
    const f = classify(runError({ stdout: long }))
  ...
  test('시그널로 죽으면 어떤 시그널인지 알려준다', () => {
    const f = classify(runError({ exitCode: null, signal: 'SIGKILL' }))
  ...
  test('입력 전달 실패는 요약이 일부만 반영됐을 수 있음을 알린다', () => {
    const f = classify(runError({ exitCode: 0, stdinFailed: true }))
    expect(f.detail).toContain('끝까지 전달하지 못했습니다')
  })

  test('타임아웃 메시지는 실제 적용된 제한 시간을 쓴다', () => {
    expect(classify(timedOut({ timeoutMs: 60_000 })).detail).toContain('60초')
  })
})

describe('classifyClaudeFailure — 로깅', () => {
  // 요약 생성과 상태 확인이 같은 분류기를 쓰므로, 로그만 보고 어느 쪽이 실패했는지 알 수 있어야 한다.
  test('context가 로그 문구에 들어간다', () => {
    classifyClaudeFailure(runError({ exitCode: 1 }), '상태 확인')
    expect(console.error).toHaveBeenCalledWith('[claude] 상태 확인 실패', expect.anything())
  })

  test('ClaudeRunError는 signal·errorCode까지 남긴다', () => {
    classify(runError({ exitCode: null, signal: 'SIGKILL' }))
  ...
  test('invalid_output도 응답 원문을 남긴다', () => {
    classify(new InvalidOutputError('사과문과 깨진 JSON'))
    expect(console.error).toHaveBeenCalledWith(
      '[claude] 요약 실패',
  ...
  test('거대한 출력은 로그에서도 잘라낸다', () => {
    classify(runError({ stdout: 'x'.repeat(50_000) }))
